...
function FlashingSpeedReading() {
  ...
  useEffect(() => {
    ...
    } else if (isReading && currentIndex >= words.length && repeat) {
      setCurrentIndex(0);
    } else {
      clearInterval(timerRef.current);
    }
    return () => clearInterval(timerRef.current);
  }, [isReading, currentIndex, words, speed, repeat]);
    ...
    <div className={`App ${isFullScreen ? 'full-screen' : ''}`}>
      ...
      <div className="word-display">
        {currentIndex < words.length && (
          <div className="current-word" style={{ wordWrap: 'break-word', overflowWrap: 'break-word' }}>
...
export default FlashingSpeedReading;
